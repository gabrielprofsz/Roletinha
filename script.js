...
const REEL_REPEATS = 6; // quantas vezes a lista inteira se repete na esteira, pra dar distância de giro

function buildReelRows(pool) {
  let html = "";
  for (let loop = 0; loop < REEL_REPEATS; loop++) {
    pool.forEach((item) => {
      html += `<div class="reel-row">${escapeXml(item.nome)}</div>`;
    });
  }
  return html;
}

function drawReelIdle(trackEl, pool) {
  trackEl.innerHTML = buildReelRows(pool);
  trackEl.style.transition = "none";
  // começa já mostrando uma volta "no meio" da lista, não sempre do primeiro item
  const meio = Math.floor((REEL_REPEATS / 2) * pool.length * REEL_ROW_HEIGHT);
  trackEl.style.transform = `translateY(-${meio}px)`;
}

function spinReelEl(trackEl, pool, selectedIndex, durationMs) {
  trackEl.innerHTML = buildReelRows(pool);

  const n = pool.length;
  const targetLoop = REEL_REPEATS - 2; // deixa uma volta de sobra depois do alvo
  const targetRow = targetLoop * n + selectedIndex;
  const windowHeight = 230; // deve bater com .reel-window no CSS
  const finalTranslate =
    targetRow * REEL_ROW_HEIGHT + REEL_ROW_HEIGHT / 2 - windowHeight / 2;
  ...
  requestAnimationFrame(() => {
    trackEl.style.transition = `transform ${durationMs}ms cubic-bezier(.12,.67,.1,1)`;
    trackEl.style.transform = `translateY(-${finalTranslate}px)`;
  });
}
...
function spinCombo() {
  ...
  sorteios.forEach(({ combo, pool, index }) => {
    spinWheelEl($(`combo-wheel-${combo.key}`), pool, index, SPIN_DURATION, combo.roletas[0], () => {
      $(`combo-wheel-${combo.key}`).style.background = buildWheelGradient(pool, combo.roletas[0]);
    });
  });
